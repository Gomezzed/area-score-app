...
export function TownHighlightsPanel({
  ...
  return (
    <div
      className={`absolute inset-y-0 right-0 w-full md:w-[28rem] bg-slate-800 border-l border-slate-700 shadow-2xl z-[1200] overflow-y-auto transition-transform duration-300 ease-out ${
    ...
    >
      <div className="p-5">
        ...
        <div className="flex items-start justify-between mb-2">
          <h2 className="flex flex-wrap items-center gap-2 text-lg font-bold text-white leading-tight">
    ...
    >
      ...
      <div className="flex items-center gap-2 px-3 py-2 bg-slate-700/40">
        ...
        <div className="text-right flex-shrink-0">
          <div className="text-[10px] text-amber-300/70 leading-none">推定 取得</div>
      ...
      <div className="px-3 py-1.5 border-l-4 border-blue-400">
        <div className="flex items-center gap-3 text-[11px]">
          <span className="text-[10px] font-bold text-blue-300">確定（前年比）</span>
      ...
      <div className="px-3 py-1.5 border-l-4 border-amber-400 bg-amber-500/5">
...
function RankTrend({ change }: { change: RankChange }) {
  ...
  if (change === 'new') {
    return (
      <span
        ...
        className="px-1 py-0.5 rounded text-[9px] font-bold bg-emerald-400/20 text-emerald-300 ring-1 ring-emerald-400/40 flex-shrink-0"
      >
        NEW
      </span>
    )
  ...
  const map = {
    up: { icon: <ArrowUp className="w-3.5 h-3.5" />, cls: 'text-emerald-400', title: '推定ランク 上昇（前月比）' },
    down: { icon: <ArrowDown className="w-3.5 h-3.5" />, cls: 'text-rose-400', title: '推定ランク 下降（前月比）' },
