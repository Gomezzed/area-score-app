...
import { ArrowLeft, Scale, Sparkles, Info, Lock } from 'lucide-react'
    ...
    <div className="bg-white border border-slate-200 rounded-xl p-4">
      ...
      <div className="flex flex-col gap-2">
        ...
        <select
          ...
          className="w-full rounded-lg bg-white border border-slate-300 text-slate-900 text-sm px-3 py-2 disabled:opacity-50 focus:outline-none focus:border-brand-700"
        >
    ...
    <div className="space-y-6">
      ...
      <section className="rounded-xl overflow-hidden border border-blue-500/30">
        <div className="bg-brand-100 border-l-4 border-brand-700 px-4 py-2">
          <h2 className="text-sm font-bold text-brand-700">確定（公表値・国勢調査）</h2>
          <p className="text-[11px] text-slate-500 mt-0.5">事実データ。両エリアを対等に比較します。</p>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-50 text-slate-500">
                ...
                <th className="text-right font-bold px-4 py-2 text-slate-900">{a?.name ?? '—'}</th>
                <th className="text-right font-bold px-4 py-2 text-slate-900">{b?.name ?? '—'}</th>
              </tr>
            </thead>
      ...
      <section className="rounded-xl overflow-hidden border border-amber-300">
        <div className="bg-[#FAEEDA]/50 border-l-4 border-amber-400 px-4 py-2 flex items-center gap-2">
          <h2 className="text-sm font-bold text-[#854F0B]">推定（町域スコア集計・ルールベース）</h2>
...
function Row({
  ...
  return (
    <tr>
      ...
      <td className={`px-4 py-2 text-right tabular-nums ${aBig ? 'text-slate-900 font-bold' : 'text-slate-600'}`}>{fmt(a)}</td>
      <td className={`px-4 py-2 text-right tabular-nums ${bBig ? 'text-slate-900 font-bold' : 'text-slate-600'}`}>{fmt(b)}</td>
    </tr>
  )
}
...
function InferredCard({ area }: { area: AreaResult | null }) {
  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-white p-3">
      <div className="text-sm font-bold text-slate-900 mb-2 truncate">{area?.name ?? '—'}</div>
