...
--  Phase 2 #7「人口推移グラフ」サンプルデータ (MVP A-1)  ⛔ 廃止・no-op 化済み
--
--  ⛔ 2026-08-11 廃止（PM裁定・PO承認済み）
--  本ファイルが投入したサンプル値は実測と最大約12倍乖離しており（名古屋市=実測の8.4%）、
--  20260811000100_purge_seed_population_history.sql で除去した。
--  本ファイルは台帳（supabase_migrations.schema_migrations）に
--  version=20260609134340（name=seed_population_history_sample）として記録される一方、
--  リポジトリのファイルから抽出される version=20260609000001 は台帳に未記録である。
--  このため db push を実行すると 20260609000001 が未適用と判定されて再実行され、
--  purge(20260811000100・記録済み=再実行されない)より低い版番号で先に流れるため、
--  「汚染だけ戻り除去は戻らない」不可逆状態を確定的に招く。
--  再汚染を防ぐため、以下のデータ書き込み（UPDATE）を全てコメントアウトして no-op 化する。
--  ファイルは Git 履歴と経緯の可読性のため削除せず残す。
--  人口推移の確定仕様＝国勢調査の実測3点（2015/2020/2025）。補間・サンプル値は使用しない。
-- ============================================================================
--
--  ↓↓↓ 以下は廃止済みの原文（全て無効化・実行されない）↓↓↓
--
